...
export default function HttpEndpointDetailsPage() {
    ...
    const endpoint = useHttpEndpointStore(
    ...
    const fetchEndpoint = useHttpEndpointStore(
        (state) => state.fetchEndpoint,
    );

    const fetchCheckResults =
        useHttpEndpointStore(
            (state) => state.fetchCheckResults,
        );

    const [statusHistoryPage, setStatusHistoryPage] =
        useState(1);

    const [checkHistoryPage, setCheckHistoryPage] =
        useState(1);
    ...
    const statusHistoryResults = useMemo(() => {
        ...
        return sortedCheckResults
            .slice(
                start,
                start + STATUS_HISTORY_PAGE_SIZE,
            )
            .reverse();
    }, [
        sortedCheckResults,
    ...
    const paginatedCheckResults = useMemo(() => {
        ...
        return sortedCheckResults.slice(
            start,
            start + CHECK_HISTORY_PAGE_SIZE,
        );
    }, [
        sortedCheckResults,
        currentCheckHistoryPage,
    ]);
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-zinc-600">
                                        {error ??
                                            "The requested HTTP endpoint could not be loaded."}
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="mb-7">
                        ...
                        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                            ...
                            <div className="flex items-center gap-2 text-xs text-zinc-600">
                                <span
                                    className={`h-1.5 w-1.5 rounded-full ${endpoint.isActive
                                        ? "bg-emerald-500"
                                        : "bg-zinc-700"
                                        }`}
                                />
                    ...
                    <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(320px,1fr)]">
                        ...
                        <section className="rounded-xl border border-zinc-900 bg-zinc-950">
                            <SectionHeader
                                ...
                                description="Availability across recent checks."
                            />

                            <div className="border-t border-zinc-900 p-5">
                                <div className="flex flex-wrap gap-1">
                                    {statusHistoryResults.length ? (
                                        statusHistoryResults.map(
                                            (result) => (
                                                <StatusBlock
                                                    key={result.id}
                                                    status={
...
function ResponseTimeChart({
    ...
    const values = points.map(
        (point) => point.responseTimeMs as number,
    );
        ...
        <div>
            <div className="mb-4 flex items-end justify-between gap-4">
                <div>
                    ...
                    <p className="mt-1 font-mono text-lg font-semibold text-zinc-200">
                        {Math.round(
                            points[points.length - 1]?.responseTimeMs as number,
                        )}{" "}
                        ms
                    </p>
                </div>
            ...
            <div className="overflow-hidden">
                ...
                >
                    ...
                    >
                        {formatTime(
                            points[points.length - 1]?.checkedAt ?? "",
                        )}
                    </text>
                </svg>
            </div>
...
function StatusBadge({
    ...
    return (
        <span
            className={`inline-flex items-center gap-1.5 text-[10px] ${isUp
                ? "text-emerald-500"
                : isPending
                    ? "text-zinc-600"
                    : "text-red-400"
        ...
        >
            <span
                className={`h-1.5 w-1.5 rounded-full ${isUp
                    ? "bg-emerald-500"
                    : isPending
                        ? "bg-zinc-700"
                        : "bg-red-400"
...
function StatusBlock({
    ...
    return (
        <div
            title={status}
            className={`h-8 flex-1 rounded-sm ${status === "UP"
                ? "bg-emerald-500/70"
                : "bg-red-500/70"
                }`}
        />
...
function ConfigRow({
    ...
    return (
        <div className="flex items-center justify-between gap-6 px-5 py-3.5">
            ...
            <span
                className={`max-w-[65%] truncate text-right text-[11px] text-zinc-400 ${mono ? "font-mono" : ""
                    }`}
            >
                {value}
            </span>
