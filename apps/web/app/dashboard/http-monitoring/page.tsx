...
export default function HttpMonitoringPage() {
    ...
    const createEndpoint = useHttpEndpointStore(
        (state) => state.createEndpoint,
    );

    const updateEndpoint = useHttpEndpointStore(
        (state) => state.updateEndpoint,
    );

    const deleteEndpoint = useHttpEndpointStore(
        (state) => state.deleteEndpoint,
    );

    const isCreating = useHttpEndpointStore(
        (state) => state.isCreating
    );

    const isUpdating = useHttpEndpointStore(
        (state) => state.isUpdating
    );

    const isDeleting = useHttpEndpointStore(
    ...
    const openEditModal = (endpoint: HttpMonitoringEndpoint) => {
        ...
        setExpectedStatusCode(String(source?.expectedStatusCode ?? endpoint.statusCode));
    ...
    const handleUpdateEndpoint = async () => {
        ...
        const name = endpointName.trim();
        const url = endpointUrl.trim();
        if (!name) { setFormError("Endpoint name is required."); return; }
        if (!url) { setFormError("Endpoint URL is required."); return; }
        try { new URL(url); } catch { setFormError("Please enter a valid URL."); return; }
        const statusCode = Number(expectedStatusCode);
        const interval = Number(intervalSeconds);
        const timeout = Number(timeoutMs);
        if (!Number.isInteger(statusCode) || statusCode < 100 || statusCode > 599) { setFormError("Status code must be between 100 and 599."); return; }
        if (!Number.isInteger(interval) || interval <= 0) { setFormError("Check interval must be greater than 0."); return; }
        if (!Number.isInteger(timeout) || timeout <= 0) { setFormError("Timeout must be greater than 0."); return; }
        const endpoint = await updateEndpoint(editingEndpointId, { name, url, method: endpointMethod, expectedStatusCode: statusCode, intervalSeconds: interval, timeoutMs: timeout });
        if (!endpoint) return;
        setCreateOpen(false);
        setEditingEndpointId(null);
        setToast({
        ...
        setEndpointName(""); setEndpointUrl(""); setEndpointMethod("GET");
        setExpectedStatusCode("200"); setIntervalSeconds("60"); setTimeoutMs("5000");
        setFormError(null);
    };
    ...
    const [method, setMethod] =
        useState<"All methods" | EndpointMethod>(
            "All methods",
        );
    const [status, setStatus] =
        useState<"All statuses" | EndpointStatus>(
            "All statuses",
        );
    ...
    const endpoints = useHttpEndpointStore(
        (state) => state.endpoints,
    );

    console.log("endpoints", endpoints);

    const isLoading = useHttpEndpointStore(
        (state) => state.isLoading,
    );

    const error = useHttpEndpointStore(
        (state) => state.error,
    );

    const fetchEndpoints = useHttpEndpointStore(
        (state) => state.fetchEndpoints,
    );
    ...
    const fetchAllCheckResults =
        useHttpEndpointStore(
    ...
    const checkResults = useHttpEndpointStore(
        (state) => state.checkResults,
    );
    ...
    useEffect(() => {
        ...
        void fetchAllLatestCheckResults(
            endpointIds,
        );

        void fetchAllCheckResults(
            endpointIds,
        );
    }, [
        endpoints,
    ...
    const filteredEndpoints =
        useMemo(() => {
            ...
            return monitoringEndpoints.filter(
                (endpoint) => {
                    const matchesSearch =
                        !query ||
                        endpoint.name
    ...
    const endpointColumns:
        ResponsiveColumn<HttpMonitoringEndpoint>[] =
        [
            {
                key: "name",
                header: "Endpoint",
            ...
            },
            {
                ...
                render: (endpoint) => (
                    <span
                        className={`inline-flex items-center gap-1.5 text-[11px] ${endpoint.status ===
                            "UP"
                            ? "text-emerald-500"
                            : "text-red-400"
                            }`}
                    >
                        <span
                            className={`h-1.5 w-1.5 rounded-full ${endpoint.status ===
                                "UP"
                                ? "bg-emerald-500"
                                : "bg-red-400"
                                }`}
            ...
            },
            {
                ...
                render: (endpoint) => (
                    <span
                        className={`font-mono text-[11px] ${endpoint.statusCode != null && endpoint.statusCode >= 400
                            ? "text-red-400"
                            : "text-zinc-500"
                            }`}
                    >
            ...
            <main className="lg:ml-64">
                <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
                        ...
                        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
                            <div>
                                ...
                                <p className="mt-1 text-sm text-zinc-600">
                                    Monitor the availability and response time of your HTTP endpoints.
                                </p>
                            </div>
                    ...
                    <div className="mt-4 flex flex-col gap-2 text-[10px] text-zinc-800 sm:flex-row sm:items-center sm:justify-between">
                        <span>
                            HTTP endpoints are loaded from the selected project.
                        </span>
                ...
                {deleteEndpointId && (
                    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget && !isDeleting) setDeleteEndpointId(null); }}>
                        <div className="w-full max-w-sm overflow-hidden rounded-xl border border-zinc-800 bg-zinc-950 shadow-2xl">
                            <div className="border-b border-zinc-900 px-5 py-4">
                                <h2 className="text-sm font-semibold text-zinc-200">Delete HTTP endpoint</h2>
                                <p className="mt-1 text-xs leading-5 text-zinc-600">This endpoint and its configuration will be removed permanently.</p>
                            </div>
                            <div className="flex items-center justify-end gap-2 px-5 py-4">
                                <button type="button" onClick={() => setDeleteEndpointId(null)} disabled={isDeleting} className="rounded-lg border border-zinc-900 bg-black px-3.5 py-2 text-xs text-zinc-500 transition-colors hover:border-zinc-800 hover:text-zinc-300 disabled:opacity-40">Cancel</button>
                                <button type="button" onClick={handleDeleteEndpoint} disabled={isDeleting} className="flex items-center gap-2 rounded-lg bg-red-500 px-3.5 py-2 text-xs font-medium text-white transition-colors hover:bg-red-400 disabled:opacity-50">
                                    {isDeleting && <span className="h-3 w-3 animate-spin rounded-full border-2 border-red-200 border-t-white" />}
            ...
            "
                        >
                            <div className="flex items-center justify-between border-b border-zinc-900 px-5 py-4">
                                <div>
                                    <h2 className="text-sm font-semibold text-zinc-200">
                                        {editingEndpointId ? "Edit HTTP endpoint" : "Add HTTP endpoint"}
                                    </h2>

                                    <p className="mt-1 text-xs text-zinc-700">
                                        {editingEndpointId ? "Update the configuration of this endpoint." : "Configure an endpoint to monitor."}
                                    </p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={(event) =>
                                            setEndpointMethod(
                                                event.target
                                                    .value as typeof endpointMethod,
                                            )
                                        }
                                        className="
                            h-10 w-full
                    ...
                    "
                                ...
                                <button
                                    type="button"
                                    onClick={editingEndpointId ? handleUpdateEndpoint : handleCreateEndpoint}
                                    disabled={isCreating || isUpdating}
                                    className="
                    ...
                    "
                                >
                                    ...
                                    {isUpdating ? "Updating..." : isCreating ? "Creating..." : editingEndpointId ? "Update endpoint" : "Create endpoint"}
                                </button>
                            </div>
                        </div>
                    </div>
...
function Toast({
    ...
    return (
        <div className="fixed right-4 top-4 z-[200] w-[min(360px,calc(100vw-2rem))]">
            <div
                className={`rounded-xl border bg-zinc-950 px-4 py-3 shadow-2xl ${type === "success"
            ...
            >
                <div className="flex items-start gap-3">
                    <span
                        className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${type === "success"
                            ? "bg-emerald-500"
                            : "bg-red-500"
                            }`}
                    />
    ...
    return (
        <div className="bg-zinc-950 p-5 transition-colors hover:bg-zinc-900/40">
            <div className="flex items-center justify-between">
                ...
                <span
                    className={`flex items-center gap-1.5 text-[10px] ${isDegraded
                        ? "text-red-500"
                        : status ===
                            "Healthy"
                            ? "text-emerald-600"
                ...
                >
                    <span
                        className={`h-1.5 w-1.5 rounded-full ${isDegraded
                            ? "bg-red-500"
                            : status ===
                                "Healthy"
                                ? "bg-emerald-500"
