...
import type { Trace as ApiTrace } from "../../../lib/api/trace.api";
...
export default function TracesPage() {
  ...
  const fetchTraces = useTraceStore((state) => state.fetchTraces);
  ...
  useEffect(() => {
    ...
    void fetchTraces(selectedProject.id);
  }, [selectedProject?.id, authStatus, fetchTraces, isAuthInitializing]);

  const uiTraces = useMemo<Trace[]>(() => {
    return traces.map((trace: ApiTrace) => {
      const durationMs = trace.durationMs ?? 0;

      return {
        id: trace.traceId || trace.id,
        service: trace.serviceName,
        operation: trace.operationName ?? "Unknown operation",
        spans: Number(trace.spanCount ?? 0),
        duration: formatDuration(durationMs),
        durationMs,
        status: trace.status === "ERROR" ? "Error" : "OK",
        time: formatRelativeTime(trace.startTime),
      };
    });
  }, [traces]);
  ...
  const services = useMemo(
    () => [
      "All services",
      ...Array.from(new Set(uiTraces.map((trace) => trace.service))),
    ],
    [uiTraces],
  );
  ...
  const filteredTraces = useMemo(() => {
    ...
    return uiTraces.filter((trace) => {
      const matchesSearch =
        !query ||
        trace.id.toLowerCase().includes(query) ||
        trace.service.toLowerCase().includes(query) ||
        trace.operation.toLowerCase().includes(query);

      const matchesService =
        service === "All services" || trace.service === service;
      ...
      const matchesDuration =
        ...
        (durationFilter === "Fast" && trace.durationMs < 100) ||
        (durationFilter === "Slow" && trace.durationMs >= 100);

      return (
        matchesSearch && matchesService && matchesStatus && matchesDuration
      );
    });
  }, [uiTraces, search, service, status, durationFilter]);
  ...
  const firstItem =
    filteredTraces.length === 0 ? 0 : (currentPage - 1) * PAGE_SIZE + 1;

  const lastItem = Math.min(currentPage * PAGE_SIZE, filteredTraces.length);
  ...
  const columns: ResponsiveColumn<Trace>[] = [
    ...
    },
    {
      ...
      render: (trace) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-zinc-400">{trace.service}</span>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      header: "Spans",
      render: (trace) => (
        <span className="text-xs text-zinc-600">{trace.spans}</span>
      ),
    },
    {
      key: "duration",
      ...
      render: (trace) => (
        <span
          className={`font-mono text-xs ${
            trace.durationMs >= 300 ? "text-amber-500" : "text-zinc-500"
          }`}
        >
          {trace.duration}
        </span>
    ...
    },
    {
      ...
      header: "Time",
      render: (trace) => (
        <span className="text-xs text-zinc-700">{trace.time}</span>
      ),
    },
  ];
    ...
    <>
        ...
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600">
              Explore requests and distributed traces across your services.
            </p>
          </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex flex-wrap items-center gap-2">
                <FilterChip
                  ...
                  options={["All durations", "Fast", "Slow"]}
                  onChange={(value) => {
              ...
              <div className="flex items-center gap-2 text-[10px] text-zinc-700">
                ...
                <span>Filters are currently applied locally.</span>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="overflow-hidden rounded-xl border border-zinc-900 bg-zinc-950">
        <div className="flex flex-col gap-3 border-b border-zinc-900 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-medium text-zinc-300">Recent traces</p>

            <p className="mt-1 text-xs text-zinc-700">
              {filteredTraces.length}{" "}
              {filteredTraces.length === 1 ? "trace" : "traces"} matched
            </p>
          </div>
        ...
        {!selectedProject?.id ? (
          <div className="flex min-h-360px flex-col items-center justify-center px-6 text-center">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-zinc-900 bg-black">
              <Waypoints className="h-4 w-4 text-zinc-700" />
            </div>
            ...
            <p className="mt-2 max-w-sm text-xs leading-5 text-zinc-700">
              Select a project to view its traces.
            </p>
          </div>
          ...
          <div className="flex min-h-360px flex-col items-center justify-center px-6 text-center">
            ...
            <p className="mt-4 text-xs text-zinc-600">Loading traces...</p>
          </div>
        ) : error ? (
          <div className="flex min-h-360px flex-col items-center justify-center px-6 text-center">
            <div className="flex h-11 w-11 items-center justify-center rounded-xl border border-zinc-900 bg-black">
          ...
          <ResponsiveDataTable
            ...
            onRowClick={(trace) => {
              window.location.href = `/dashboard/traces/${trace.id}`;
            }}
          />
        ) : (
          <EmptyTraceState
        ...
        {filteredTraces.length > 0 && (
          <div className="flex flex-col gap-3 border-t border-zinc-900 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs text-zinc-700">
              ...
              of <span className="text-zinc-500">{filteredTraces.length}</span>
            </p>
      ...
      <div className="mt-4 flex flex-col gap-2 text-[10px] text-zinc-800 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <span>
            Trace data updates automatically when live ingestion is enabled.
          </span>
        </div>
...
function FilterChip({
  ...
  return (
    <label className="relative flex items-center">
      <span className="mr-2 text-[10px] text-zinc-700">{label}</span>
